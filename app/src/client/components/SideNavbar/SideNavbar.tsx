...
import { HiOutlineShoppingBag } from 'react-icons/hi'; // Icon for Products
import { IoIosArrowBack } from 'react-icons/io'; // Collapse icon
import { cn } from '../../cn';
      ...
      <div className="flex flex-col h-full">
        ...
        <nav className="flex-1 pt-6">
          <ul className="space-y-2 px-4">
            ...
            {/* Step 1: Add RatePlans Link */}
            <li>
              <Link
                to="/rate-plans" // Route for RatePlans (you'll define this later in main.wasp)
                className={cn(
                  'flex items-center px-4 py-3 text-gray-600 dark:text-gray-300 rounded-lg transition-colors group',
                  {
                    'bg-gray-100 dark:bg-gray-700 text-blue-600 dark:text-white': isActive('/rate-plans'),
                    'hover:bg-gray-100 dark:hover:bg-gray-700': !isActive('/rate-plans'),
                  }
                )}
              >
                {/* Step 2: Use an icon for RatePlans (replace HiOutlineShoppingBag with a suitable icon) */}
                <HiOutlineShoppingBag className={cn('h-6 w-6', {
                  'text-blue-600 dark:text-white': isActive('/rate-plans'),
                })} />
                <span
                  className={cn('ml-3 font-medium transition-opacity duration-300', {
                    'opacity-0 hidden': isCollapsed,
